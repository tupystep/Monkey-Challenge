...
def write_labels(cells: list[tuple[float, float]], cell_size: float, patch_size: int, label_file: TextIO,
    ...
    for roi in lymph_annot['rois']:
        ...
        for i, (x, y) in enumerate(coverage):
            ...
            with open(labels_dir / label_name, 'x') as label_file:
                write_labels(lymph_split[i], lymph_size, patch_size, label_file, class_label=0)
                write_labels(mono_split[i], mono_size, patch_size, label_file, class_label=1)
