...
def compare_patches(patches: list[np.ndarray], title: str) -> int:
    ...
    if img is not None and len(rows) > 0:
...
def compare_boxes(datasets: list[str], draw_dots: bool = True) -> None:
    ...
    images = list((Path('data') / datasets[0] / 'images').glob('*.png'))
    while True:
        patch_name = np.random.choice(images).name
        patches = []
        for dataset in datasets:
...
def compare_models(dataset: str, models: list[str], iou: float) -> None:
    ...
    while True:
        ...
        for model_name in models:
            ...
            results = model.predict(patch_path, imgsz=imgsz, conf=0.25, max_det=None, agnostic_nms=True, iou=iou, verbose=False)
            pred_patch = draw_predictions(patch_path, results[0])
            patches.append(pred_patch)
        title = 'TRAIN ' + patch_path.name if patch_path in train_images else 'VAL ' + patch_path.name
        key = compare_patches(patches, title)
...
if __name__ == '__main__':
    # compare_boxes(['basic_box/pas-cpg512', 'seg_box/pas-cpg512', 'pure_seg_box/pas-cpg512'])
    ...
    compare_models('seg_box/pas-cpg256', ['yolo/basic_box/default_aug/img256_ep200_yolo11m/weights/last.pt',
                    'yolo/basic_box/img256_ep200_yolo11m/weights/last.pt'], iou=0.7)
