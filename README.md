...
are modifiable using CLI arguments, with `-h` printing their description,
...
#### Data visualization script `display_annot.py`
`python display_annot.py data/raw/images/pas-cpg`

Script `evaluate.py` evaluates all trained models with their runs and
...
`test/input/images/tissue-mask` with a corresponding tissue mask, and `test/model` with model
